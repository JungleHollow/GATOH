...
class IterationsTester:
    ...
    For this experiment, all models will be identical; the purpose being to inspect the level of convergence
    achieved by each model at varying numbers of maximum iterations.

    A secondary objective being a characterisation of the relationship between number of model iterations versus
    the repetitions of models at the same iterations; in order to determine the tradeoff between longer-running
    models, or shorter models with a greater number of repetitions being run.

    This experiment specifically is an extension of the original NumIterations, using GATOH's Groups module and
    clustered simulations rather than individual agents; done in order to ensure that the impact of model iterations
    ...
    def create_models(self) -> None:
        ...
        """
        for iteration in TEST_PARAMETERS["iterations"]:
            for i in range(TEST_PARAMETERS["repetitions"]):
                model_id: str = f"{TEST_PARAMETERS['model_id_base']}-{iteration:03}ITERS-{i + 1:03}"
...
if __name__ == "__main__":
    ...
    TEST_PARAMETERS: TestParameters = {
        "iterations": [i + 1 for i in range(100)],
        "repetitions": 10,
    ...
    AGENT_PARAMETERS: AgentParameters = {
        ...
        "id_base": "GSANIA"  # (Group Stochastic Analysis Number Iterations Agent)
    }
    ...
    GROUP_PARAMETERS: GroupParameters = {
        "n_groups": 4,
        "id_base": "GSANIG",  # (Group Stochastic Analysis Number Iterations Group)
    }

    # The root directory of the experiment itself
    ROOT_DIR: str = "./experiments/StochasticAnalysis/GroupNumIterations"
    ...
    if directory_missing:
        ...
        results = AnalysisResults()
        tester = IterationsTester(results)
        ...
        print("Loading a saved, previously run instance of the Group NumIterations experiment...")
        results = AnalysisResults()
        tester = IterationsTester(results, existing=True)
        tester.load_models()
        tester.load_results()
